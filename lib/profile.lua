...
function profile.query(limit)
    ...
    for i, f in ipairs(t) do
        ...
        local str = string.format("%.3f", _telapsed[f] + dt)
        t[i] = {i, _labeled[f] or "?", _ncalls[f], str, _defined[f]}
    end
    return t
end

local cols = {2, 20, 11, 8, 28}
...
function profile.report(n)
    ...
    local function centerString(s, width)
        local len = s:len()
        if len >= width then
            return s
        end
    ...
    for i, row in ipairs(report) do
        for j = 1, 5 do
            ...
            local maxWidth = cols[j]

            if j == 2 and s:len() > maxWidth then
                s = s:sub(1, maxWidth - 3) .. "..." -- Truncate with ellipsis if too long
            else
                s = centerString(s, maxWidth) -- Center if too short
            end
    ...
    -- Rows to separate header and data
    local divider = "+----+----------------------+-------------+----------+------------------------------+ \n"
    -- Creating the header row
    local headers = {"#", "Function", "Calls", "Time (s)", "Location"}
    local headerRow = "|"
    for j, header in ipairs(headers) do
        headerRow = headerRow .. " " .. centerString(header, cols[j]) .. " |"
    end
    local sz = divider .. headerRow .. "\n" .. divider
    if #out > 0 then
